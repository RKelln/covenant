# Credits & Colophon

**Covenant** is an open-source, living document.
It was authored collaboratively between human creators and emerging machine intelligences.

## Initial Steward
Ryan Kelln

## Contributing Intelligences
Claude, GPT, and Gemini models

## Typesetting, Layout, & Design
Ryan Kelln, Gemini 3.1 Pro and Claude Sonnet 4.6 via GitHub Copilot

Inspired and made in part from 
[Anthropic's Claude Constitution](https://www.anthropic.com/constitution).

---

*Set in [Cormorant Garamond](https://github.com/CatharsisFonts/Cormorant),*
*designed by Christian Thalmann (Catharsis Fonts),*
*released under the SIL Open Font License.*

*Generated via WeasyPrint.*
